...
# Find position after id: 9 block
# Search for id: 9, 
session_9_pos = orig_content.find("        id: 9,")
if session_9_pos != -1:
    # Find the end of this session object
    # We look for the closing ] for lessons, then the closing } for session
    search_start = orig_content.find("lessons: [", session_9_pos)
    
    # Simple brackets matching for the lessons array
    lessons_end = -1
    balance = 0
    for i in range(search_start + 9, len(orig_content)):
        if orig_content[i] == '[': balance += 1
        elif orig_content[i] == ']':
            if balance == 0:
                lessons_end = i
                break
    ...
    if lessons_end != -1:
        session_end = orig_content.find("},", lessons_end)
        if session_end != -1:
            insert_pos = session_end + 2
            new_content = orig_content[:insert_pos] + "\n" + good_session_text + orig_content[insert_pos:]
            with open(lessons_file, "w", encoding="utf-8") as f:
                f.write(new_content)
            print("lessons.js updated with Word Advanced after Session 9!")
        else:
            print("Could not find end of Session 9 object")
    else:
        print("Could not find end of Session 9 lessons array")
else:
    print("Could not find Session 9 in lessons.js")
