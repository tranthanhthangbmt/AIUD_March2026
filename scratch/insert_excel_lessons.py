...
# ========== STEP 2: Read HTML content and build lesson JS entries ==========
...
def read_html_content(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        html = f.read()
    # Extract content between <div class="lesson-content"> and </div>
...
def build_lesson_html_content(base_dir, lesson_id, slug, category):
    """Build the HTML content string for a lesson, with proper image paths."""
    ...
    # Fix image src paths to be relative to Module_1-6
    content = content.replace(
        'src="images/', 
        f'src="Module_1-6/TaiLieuHuongDan/Excel/{category}/images/'
    )
    
    # Fix practice file paths
    content = content.replace(
        'href="practice_files/',
        f'href="Module_1-6/TaiLieuHuongDan/Excel/{category}/practice_files/'
    )
    
    # Add inline styles to images (matching Word pattern)
    content = re.sub(
        r'<img\s+(?!style)',
...
def split_content_into_steps(html_content):
    """Split HTML content into steps based on <h4> headings, similar to Word lessons."""
    # Split by <h4> tags
    ...
    # If no steps were created, put everything in one step
    if not steps:
...
def generate_lesson_entry(lesson_id, slug, title_vi, session_id, prefix, base_dir, category):
    """Generate a single lesson JS object."""
    ...
    if os.path.exists(practice_dir):
        for f in os.listdir(practice_dir):
            if slug.replace('-', '') in f.replace('-', '').replace('_', '').lower() or \
               f"lesson{lesson_id}" in f.replace('_', '').lower():
    ...
    entry = f'''            {{
                ...
                description: "Nội dung chi tiết cho bài Bài {lesson_id}: {escape_js_string(title_vi)}.",
                tasks: {tasks_str},
                steps: {steps_str}
            }}'''
...
def generate_section(lessons_list, session_id, prefix, base_dir, category, section_title, section_desc):
    """Generate a full section (Excel Cơ Bản or Excel Nâng Cao)."""
    lesson_entries = []
...
# Find the position of "Buổi 10: Hàm trong Excel" section
# We need to insert Excel Cơ Bản right before the { that starts Buổi 10
buoi10_pattern = r'(\n    \{\n\s*id: 10,\n\s*title: "Buổi 10: Hàm trong Excel")'
buoi10_match = re.search(buoi10_pattern, content)

if buoi10_match:
    insert_pos = buoi10_match.start()
    content = content[:insert_pos] + "\n" + excel_basic_section + "," + content[insert_pos:]
    print("\n✅ Inserted 'Excel Cơ Bản' before Buổi 10")
else:
    print("\n❌ Could not find Buổi 10 insertion point!")

# Find the position of "Buổi 12: AI for Office" section
# We need to insert Excel Nâng Cao right before Buổi 12
buoi12_pattern = r'(\n    \{\n\s*id: 12,\n\s*title: "Buổi 12: AI for Office)'
buoi12_match = re.search(buoi12_pattern, content)

if buoi12_match:
    insert_pos = buoi12_match.start()
    content = content[:insert_pos] + "\n" + excel_adv_section + "," + content[insert_pos:]
    print("✅ Inserted 'Excel Nâng Cao' before Buổi 12")
else:
    print("❌ Could not find Buổi 12 insertion point!")

# Write updated lessons.js
with open(lessons_js_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("\n🎉 lessons.js updated successfully!")
